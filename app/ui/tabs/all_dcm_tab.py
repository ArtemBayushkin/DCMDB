...
from PyQt6.QtWidgets import (
    QPushButton, QTableView, QHeaderView, QAbstractItemView, QHBoxLayout, QLineEdit)
...
class AllDcmTab(BaseTab):
    def __init__(self, main_window=None):
        super().__init__("Все вопросы в работе", icon="📋", space=0)
        self.model = None
        self.table = None
    ...
    def load_data(self):
        with DcmManager() as mgr:
            ...
            model = mgr.get_query_model(
                limit=3000,
                need_clarification=False,
                # archive=False,               # если нужно
                # is_urgent=None,
                # designer="Баюшкин",          # пример
                columns=[
                    "ID", "Date of meeting", "Urgent/Срочный",
                    "Code of the WD or MD", "Description of problem",
                    "Appendix", "Symbols of decisions under the Protocol",
                    "Текст решения, дата", "Texts of  decisions, date",
                    "Desighner's surname", "Приложение", "Заметка"
                ]
            )

            if model.rowCount() == 0 and model.lastError().isValid():
